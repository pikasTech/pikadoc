...
本文档中通过编写构造器函数的方式实现一个 `class` 在实际开发中还是略显麻烦，PikaScript 提供了自动生成构造器函数和自动构造根对象的工具，在后续的开发中，只要用 Python 的语法声明一个 `class` 就能达到手动编写构造器函数一样的效果，详情请看 [C 模块-> 将 C 库制作为 Python 库](https://pikadoc.readthedocs.io/zh/latest/index_cmodule.html)
